<? // pr($chart);?>
<h1><?=$genre?> Song Chart Preview</h1> 
...
<table>
	...
	<tr>
		<td><?=$i;?></td>
		<td <? 
			if(!$entry['Song']['verified']) echo 'style="color:#0ff;"';
			else if(!$entry['Song']['approved']) echo 'style="color:#f00;"';
			?>><?=$entry['Song']['name'];
			?></td>
		<td <? 
			if(!$entry['Song']['Band']['verified']) echo 'style="color:#0ff;"';
