...
<p class="form_subtitle">Contact Information for the Station or Show:</p><hr />
	   ...
	   <? array_pop($countries); ?>
	   <?=$this->Form->input('RadioAddress.0.country_id', array('options' => $countries, 'empty' => '(choose one)')); ?>
	   <?=$this->Format->address_form($country['Country']['address_format'],'RadioAddress.0'); ?>
</p>
<?=$this->Form->input('id'); ?>
<?=$this->Form->input('RadioTerrestrialDetail.id'); ?>
<?=$this->Form->input('RadioInternetDetail.id'); ?>
<?=$this->Form->input('RadioSyndicatedDetail.id'); ?>
<?=$this->Form->input('RadioSatelliteDetail.id'); ?>
<?=$this->Form->input('RadioPhoneNumber.0.id'); ?>
<?=$this->Form->input('RadioAddress.0.id'); ?>
<?=$this->Form->input('RadioLink.0.id'); ?>
<?=$this->Form->input('RadioEmail.0.id'); ?>

<?=$this->Form->end('Make Changes'); ?>