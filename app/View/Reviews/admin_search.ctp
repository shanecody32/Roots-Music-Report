...
<section id="results">
	...
	<? $i=0; foreach($reviews as $review): ?>
		<? if($i % 2 == 0){ echo '<tr>'; } ?>
			<td id="album_<?=$i;?>"  class="each-album<? if(!$review['Review']['approved'])echo ' not_approved'; ?>">
				<? if(!$review['Review']['approved']){ ?>
					<?=$this->Image->show($review['Album']['AlbumImage'], array('class'=>'album-front-cover', 'url'=>array('controller'=>'reviews', 'action'=>'approve', $review['Review']['id'], 'admin'=>true)), 100, 100); ?>
				<? } else { ?>
				...
				<article class="album-info">
					...
					<? if(!$review['Review']['approved']){ ?>
						<?=$this->Html->link($review['Band']['name'], array('controller'=>'reviews', 'action'=>'approve', $review['Review']['id'], 'admin'=>true)); ?>
					<? } else { ?>
						<?=$this->Html->link($review['Band']['name'], array('controller'=>'reviews', 'action'=>'view', $review['Review']['id'], 'admin'=>false)); ?>
					<? } ?><br />
