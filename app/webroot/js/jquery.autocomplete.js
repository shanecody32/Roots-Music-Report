...
$(document).ready(function(){
	...
	$("#BandName").autocomplete({
		source: function(request, response){
			$.post(dir+"auto_completes/band_autocomplete", {term:request.term}, function(data){
		...
		select: function(event, ui) {
			...
			for(var index = 0; index < ui.item.album.length; index++){
				for(var index2 = 0; index2 < ui.item.album[index].Song.length; index2++){
					html += '<tr><td>';
					html += '<div class="input checkbox" style="display:inline;"><input type="hidden" name="data[RadioStaffPlaylist][AddSong]['+index2+'][check]" id="RadioStaffPlaylistAddSong'+index2+'Check_" value="0"><input type="checkbox" name="data[RadioStaffPlaylist][AddSong]['+index2+'][check]" value="1" id="RadioStaffPlaylistAddSong'+index2+'Check"><label for="RadioStaffPlaylistAddSong'+index2+'Check"></label></div>';
					html += '</td><td>';
					html += ui.item.album[index].Song[index2].name;
					html += '<input type="hidden" name="data[RadioStaffPlaylist][AddSong]['+index2+'][song_id]" id="RadioStaffPlaylistAddSong'+index2+'SongId" value="'+ui.item.album[index].Song[index2].id+'">';
					html += '<input type="hidden" name="data[RadioStaffPlaylist][AddSong]['+index2+'][album_id]" id="RadioStaffPlaylistAddSong'+index2+'AlbumId" value="'+ui.item.album[index].id+'">';
					html += '</td><td>';
					html += ui.item.album[index].name;
					html += '</td><td>';
					html += ui.item.album[index].Label.name;
					html += '</td><td>';
					html += ' <input style="display:inline; width:30px; height:12px;" name="data[RadioStaffPlaylist][AddSong]['+index2+'][new_spins]" type="text" value="0" id="RadioStaffPlaylistAddSongNewSpins">';
					html += '</td></tr>';
				}
			}
			html += '</table>';
			...
			$('#BandLinkLink').val(ui.item.website);
	   ...
	   $("#LabelName").autocomplete({
		source: function(request, response){
			$.post(dir+"auto_completes/label_autocomplete", {term:request.term}, function(data){
				response($.map(data, function(item) {
				return {
					label: item.name,
					value: item.id
	   ...
	   $("#AlbumName").autocomplete({
		source: function(request, response){
			$.post(dir+"auto_completes/album_autocomplete/"+band, {term:request.term}, function(data){
				response($.map(data, function(item) {
				return {
					label: item.name,
					value: item.id,
	   ...
	   $("#SongName").autocomplete({
		source: function(request, response){
			$.post(dir+"auto_completes/song_autocomplete/"+band, {term:request.term}, function(data){
				response($.map(data, function(item) {
				return {
					label: item.name,
					value: item.id,
	...
	$("#UserAddressState").autocomplete({
		source: function(request, response){
			$.post(dir+"../auto_completes/state_autocomplete/"+$("#UserAddressCountryId").val(), {term:request.term}, function(data){
				response($.map(data, function(item) {
				return {
					label: item.name,
					value: item.id,
	...
	$("#RadioAddress0State").autocomplete({
		source: function(request, response){
			$.post(dir+"../auto_completes/state_autocomplete/"+$("#RadioAddress0CountryId").val(), {term:request.term}, function(data){
				response($.map(data, function(item) {
				return {
					label: item.name,
					value: item.id,
