...
$ext = strtolower(substr(basename($image), strrpos(basename($image), ".") + 1));
    ...
    function resizeThumbnailImage($thumb_image_name, $image, $width, $height, $start_width, $start_height, $scale){
	   $newImageWidth = ceil($width * $scale);
	   $newImageHeight = ceil($height * $scale);
	   if($newImageHeight > 1.5 * $width){
		  $newImageHeight = ceil($width * $scale *1.5);
	   }
	   $newImage = imagecreatetruecolor($newImageWidth,$newImageHeight);
	   $ext = strtolower(substr(basename($image), strrpos(basename($image), ".") + 1));
	   $source = "";
    ...
    function cropImage($thumb_width, $x1, $y1, $x2, $y2, $w, $h, $thumbLocation, $imageLocation){
	   $scale = $thumb_width/$w;
	   $cropped = $this->resizeThumbnailImage(WWW_ROOT.DS.'img'.DS.str_replace("/", DS,$thumbLocation),WWW_ROOT.DS.'img'.DS.str_replace("/", DS,$imageLocation),$w,$h,$x1,$y1,$scale);
	   return $cropped;
    }
}
?>