...
<div id="results">
	...
	<table>
	<?=$this->Html->tableHeaders(array($this->Paginator->sort('name', 'Name '),'Type', ''));?>
	<? $i = 0; foreach($stations as $radio): ?>
	<?
		$actions = $this->Html->link(__('View Reporters'), array('controller'=>'radio_staff_members', 'action' => 'view_by_radio', $radio['RadioStation']['id'])).' | ';
		//$actions .= $this->Html->link(__('Playlist'), array('controller'=>'radio_station_playlists','action' => 'admin_view', $radio['RadioStation']['id'])).' | ';
		$actions .= $this->Display->actions($radio['RadioStation']['id'], array('view' => "View Profile"), false);
	?>
		<?=$this->Html->tableCells(array($radio['RadioStation']['name'], $radio['RadioStation']['type'],$actions)); ?>
	<? endforeach; ?>
	</table>
